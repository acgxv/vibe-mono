...
Sources: `README.md`, `Cargo.toml`, `Cargo.lock`, `apps/name_placeholder/Cargo.toml`, `apps/name_placeholder/README.md`, `apps/name_placeholder/src/main.rs`, `apps/name_placeholder/src/cli.rs`, `.github/workflows/language.yml`, `.github/workflows/release.yml`; migration rationale and history are retained in [Knowledge Maintenance](knowledge-maintenance.md).

## 1. Establish Identity And Inventory

Search tracked source for at least:

```sh
rg -n 'name_placeholder|description_placeholder|hack-ink/name_placeholder|Welcome to use' \
  README.md Cargo.toml Cargo.lock apps .github openwiki
```

Do not search ignored secret-bearing files. Decide the canonical Cargo package/binary name, app directory name, repository URLs, package description, owners, and platform data identity before replacing anything.
...
- The workspace changes from `apps/*`, gains shared packages, or adds another language toolchain.
